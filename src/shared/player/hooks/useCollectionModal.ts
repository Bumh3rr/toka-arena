...
export function useCollectionModal() {
  ...
  const { data, error, mutate, isValidating } = useSWR<CollectionModalData>(
    ['player.collection-modal', page, PAGE_SIZE],
    async () => {
  ...
  const activate = useCallback(async (id: string) => {
    ...
    // 'player' es la clave compartida del PlayerProfile (Arena, Colección, Tienda).
    // Sin ella el tokagotchi activo queda stale fuera de Home — es el BUG-1.
    await Promise.all([globalMutate('player'), globalMutate('home')])
